...
from AutomatedTesting.Instruments.TopLevel.ProcessWIthCleanStop import (
    ProcessWithCleanStop,
)
...
class InstrumentChannel:
    ...
    def enable_output(self, enabled=True):
        ...
        if enabled:
            logging.debug(
                f"{self.instrument.name}, " f"Channel {self.get_name()}: Output Enabled"
            )
            self.errorThread = ProcessWithCleanStop(target=self.check_for_errors)
    ...
    def cleanup(self):
        ...
        logging.debug(f"{self.instrument.name}, " f"Channel {self.get_name()} shutdown")


class MultiChannelInstrument(BaseInstrument):
    def __init__(self, address, id, name, channelCount, channels, *args, **kwargs):
    ...
    def validate_channel_number(self, channelNumber):
        ...
        if (
            ...
            or channelNumber < 0
            or channelNumber > self.channelCount
        ):
            raise ValueError(
                f"Invalid channel number {channelNumber} for " f"{self.name}"
            )
    ...
    def reserve_channel(self, channelNumber: int, name: str) -> InstrumentChannel:
        ...
        logging.info(f"{self.name}, Channel {channelNumber} " f'reserved for "{name}"')
