...
class RandS_SMB100A(SignalGenerator):
    ...
    def __init__(self, address, name="SMB100A"):
        ...
        channel1 = SignalGeneratorChannel(
            channelNumber=1, maxPower=30, minPower=-145, maxFreq=12.75e9, minFreq=100e3
        )

        super().__init__(
            id="Rohde&Schwarz,SMB100A,1406.6000k03/180437," "3.1.19.15-3.20.390.24",
            name=name,
            channelCount=1,
            channels=[channel1],
            address=address,
    ...
    def enable_channel_output(self, channelNumber, enabled):
        ...
        self._write(f"OUTP {1 if bool(enabled) else 0}", acquireLock=True)
