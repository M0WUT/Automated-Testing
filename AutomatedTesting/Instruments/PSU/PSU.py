...
from AutomatedTesting.Instruments.TopLevel.MultiChannelInstrument import (
    InstrumentChannel,
...
class PowerSupply(MultiChannelInstrument):
    ...
    def __init__(self, address, id, name, channelCount, channels, **kwargs):
        super().__init__(address, id, name, channelCount, channels, **kwargs)
...
class PowerSupplyChannel(InstrumentChannel):
    ...
    def __init__(self, channelNumber, maxVoltage, minVoltage, maxCurrent, minCurrent):
    ...
    def set_voltage_limits(self, minVoltage, maxVoltage):
        ...
        Raises:
            ValueError: If requested limits are outside
                instrument limits
        """
        if (maxVoltage > self.absoluteMaxVoltage) or (
    ...
    def set_current_limits(self, minCurrent, maxCurrent):
        ...
        Raises:
            ValueError: If requested limits are outside
                instrument limits
        """
        if (maxCurrent > self.absoluteMaxCurrent) or (
    ...
    def set_voltage(self, voltage):
        ...
        if voltage != self.voltageSetpoint:
            if self.minVoltage <= voltage <= self.maxVoltage:
                ...
                x = self.read_voltage_setpoint()
                if x != voltage:
                    raise Exception(
                        f"{self.instrument.name}, "
                        f"Channel {self.channelNumber}"
                        f" requested voltage of {voltage}V failed."
                        f" Readback {x}V"
                    )
    ...
    def read_voltage_setpoint(self):
        ...
        Raises:
            None
        """
        return self.instrument.read_channel_voltage_setpoint(self.channelNumber)
    ...
    def read_current_setpoint(self):
        ...
        Raises:
            None
        """
        return self.instrument.read_channel_current_setpoint(self.channelNumber)
    ...
    def measure_current(self):
        ...
        logging.debug(
            f"{self.instrument.name}, Channel {self.name} " f"current measured as {x}A"
        )
        return x
    ...
    def enable_ovp(self, enabled=True):
        ...
        if enabled:
            logging.debug(
                f"{self.instrument.name}, " f"Channel {self.name}: OVP Enabled"
            )
        else:
            logging.debug(
                f"{self.instrument.name}, " f"Channel {self.name}: OVP Disabled"
            )
    ...
    def enable_ocp(self, enabled=True):
        ...
        if enabled:
            logging.debug(
                f"{self.instrument.name}, " f"Channel {self.name}: OCP Enabled"
            )
        else:
            logging.debug(
                f"{self.instrument.name}, " f"Channel {self.name}: OCP Disabled"
            )
