...
from AutomatedTesting.PytestDefinitions.ResultsHandler import (
    MeasurementPoint,
    ResultsHandler,
)
...
class TestSupervisor:
    def __init__(
        self, loggingLevel, instruments, saveResults=True, calibrationPower=-30
    ):
        logging.getLogger("pyvisa").setLevel(logging.WARNING)
        if saveResults is True:
            self.name = input("Please enter DUT name and press Enter...\n")
            while os.path.isdir(f"./Measurements/{self.name}"):
                self.name = input("Name already taken, please enter another...\n")
        ...
        self.instrumentSupervisor.request_resources(self.instruments)
    ...
    def request_measurements(self, measurements):
        ...
        for x in measurements:
            if x not in self.requestedMeasurements:
                ...
                assert (
                    self.setup.dutLimits.minFreq <= x.freq
                ...
                if x.drainVoltage:
                    ...
                    assert x.drainVoltage <= self.setup.dutLimits.maxDrainVoltage
                ...
                self.requestedMeasurements[index].measureCurrent |= x.measureCurrent
    ...
    def run_measurements(self):
        ...
        for x in self.requestedMeasurements:
            ...
            x.outputPower = self.signalSink.measure_power(x.freq)
